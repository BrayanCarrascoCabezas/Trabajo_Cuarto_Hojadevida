...
public class Exp_DocenteImpl implements IExp_Docente{
    ...
    public int insertar(Exp_Docente exp_exp_docenteente) throws Exception {
        int nFilas = 0;
        String csql = "INSERT INTO Exp_Docente (Codigo_Ex_D, codigoE,  institucion, fechainicio, fechafinal) VALUES (?,?,?,?,?)";
        ...
        //lstP.add(new Parametro(3, exp_exp_docenteente.getCodCursoM()));
        lstP.add(new Parametro(3, exp_exp_docenteente.getInstitucion()));
        lstP.add(new Parametro(4, exp_exp_docenteente.getFechainicio()));
        lstP.add(new Parametro(5, exp_exp_docenteente.getFechafinal()));
        Conexion con = null;
        try {
            con = new Conexion();
            con.conectar();
    ...
    public int modificar(Exp_Docente exp_exp_docenteente) throws Exception {
        int nFilas=0;
        String csql="UPDATE Exp_Docente SET Codigo_Ex_D=?, codigoE=?,  institucion=?, fechainicio=?, fechafinal=? Where Codigo_Ex_D=?";
...
//        lstPar.add(new Parametro(3, exp_exp_docenteente.getCodCursoM()));
        lstPar.add(new Parametro(3, exp_exp_docenteente.getInstitucion()));
        lstPar.add(new Parametro(4, exp_exp_docenteente.getFechainicio()));
        lstPar.add(new Parametro(5, exp_exp_docenteente.getFechafinal()));
        Conexion con=null;
        try {
            con=new Conexion();
            con.conectar();
    ...
    public Exp_Docente obtener(int codigo) throws Exception {
        ...
        String csql = "SELECT codExpD, codE, codCursoM, institucion, fechainicio, fechafinal FROM Exp_Docente Where codExpD=?";
        ArrayList<Parametro> lstPar = new ArrayList<>();
...
//                exp_docente.setCodCursoM(rst.getInt(3));
                exp_docente.setInstitucion(rst.getString(3));
                exp_docente.setFechainicio(rst.getDate(4));
                exp_docente.setFechafinal(rst.getDate(5));
            }
        } catch (Exception e) {
            System.out.println("Error: " + e.getMessage() + " " + e.getLocalizedMessage());
        } finally {
    ...
    public ArrayList<Exp_Docente> obtener() throws Exception {
        ...
        IExperiencia expDao=new ExperienciaImpl();
        Experiencia exp=null;
        String csql="SELECT Codigo_Ex_D, codigoE, institucion, fechainicio, fechafin from Exp_Docente";
        Conexion con=null;
        try {
            con=new Conexion();
            con.conectar();
